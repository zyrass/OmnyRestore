...
erDiagram
    ...
    ORDERS {
        ...
        text description "Client restoration instructions"
    ...
    MEDIA {
        ...
        string path
        bigint size
        json custom_properties "AI metadata, restoration parameters"
        timestamp created_at
    }
...
sequenceDiagram
    actor Client
    participant Browser
    participant Livewire as Livewire<br/>OrderCreateForm
    participant Laravel
    participant S3
    ...
    Client->>Browser: Fill form + upload photos (multi-file)
    Browser->>Livewire: wire:model + chunked upload
    Livewire->>S3: Store to temporary disk (disk:temp)
    S3-->>Livewire: Temporary path confirmed

    Client->>Browser: Click "Submit my order"
    Browser->>Livewire: submit()
    Livewire->>Laravel: OrderController@store()

    Note over Laravel,DB: Validation: MIME type, size, count

    Laravel->>DB: INSERT orders (status = PENDING)
    Laravel->>S3: Move photos temp → disk:s3/originals/{order_id}/
    Laravel->>DB: INSERT media (collection = originals)
    Laravel->>Queue: Dispatch NotifyAdminNewOrderJob
    Queue->>Mail: Email admin — New order received

    Laravel-->>Browser: Redirect /client/orders/{id}
    Browser-->>Client: Tracking page — status: PENDING
```
...
sequenceDiagram
    actor Admin
    participant Browser
    participant Livewire as Livewire<br/>AdminOrderManage
    participant Laravel
    participant OpenAI as OpenAI API<br/>GPT-4o Vision
    participant S3
    participant DB
    participant Queue
    participant Mail

    Admin->>Browser: Open order from dashboard
    Browser->>Livewire: Display original photos grid
    Admin->>Browser: Click "Take charge"
    Livewire->>Laravel: PATCH /admin/orders/{id}/status
    Laravel->>DB: UPDATE status = IN_PROGRESS
    Laravel->>Queue: Dispatch NotifyClientStatusJob
    Queue->>Mail: Email client — Order in progress

    Note over Admin,OpenAI: Admin downloads originals, runs AI restoration prompt<br/>(ChatGPT 8K upscale / colorization prompt)

    Admin->>Browser: Upload AI-restored photos
    Livewire->>S3: disk:s3/retouched/{order_id}/

    Note over Laravel,S3: Auto-generate watermarked preview (low-res + overlay)

    Admin->>Browser: Click "Mark as done"
    Livewire->>Laravel: PATCH status = DONE
    Laravel->>DB: UPDATE status + delivered_at
    Laravel->>Queue: Dispatch GenerateOrderZipJob
    Queue->>S3: Read retouched photos
    Queue->>S3: Write ZIP (disk:s3/deliveries/{order_id}.zip)
    Queue->>DB: INSERT order_deliveries
    Laravel->>Queue: Dispatch NotifyClientPaymentJob
    Queue->>Mail: Email client — Photos ready, payment link
```
